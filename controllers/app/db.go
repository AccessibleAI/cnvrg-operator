...
func (r *CnvrgAppReconciler) getEsCredsSecret(app *mlopsv1.CnvrgApp) (user string, pass string, err error) {
	...
	if err := r.Get(context.Background(), namespacedName, &creds); err != nil && errors.IsNotFound(err) {
		log.Error(err, "es-creds secret not found!")
		return "", "", err
	} else if err != nil {
		log.Error(err, "can't check if es creds secret exists", "name", namespacedName.Name)
		return "", "", err
	}
...
func (r *CnvrgAppReconciler) dbsState(app *mlopsv1.CnvrgApp) error {
	...
	if app.Spec.Dbs.Es.Enabled {
		...
		log.Info("trying to generate es creds (if still doesn't exists...)")
		if err := desired.Apply(dbs.EsCreds(esSecretData), app, r.Client, r.Scheme, log); err != nil {
		...
		if err := desired.Apply(dbs.KibanaConfSecret(*kibanaConfigSecretData), app, r.Client, r.Scheme, log); err != nil {
			r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
			return err
		}
		if err := desired.Apply(dbs.KibanaState(app), app, r.Client, r.Scheme, log); err != nil {
			r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
			return err
		}
	...
	if app.Spec.Dbs.Pg.Enabled {
		...
		log.Info("trying to generate pg creds (if still doesn't exists...)")
		if err := desired.Apply(dbs.PgCreds(pgSecretData), app, r.Client, r.Scheme, log); err != nil {
			r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
			return err
		}
	}
	...
	if app.Spec.Dbs.Redis.Enabled {
		...
		log.Info("trying to generate redis creds (if still doesn't exists...)")
		if err := desired.Apply(dbs.RedisCreds(redisSecretData), app, r.Client, r.Scheme, log); err != nil {
			r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
			return err
		}
	}
	...
	if app.Spec.Dbs.Prom.Enabled {
		...
		if _, _, passHash, err := desired.GetPromCredsSecret("prom-creds", app.Namespace, r.Client, log); err != nil {
			r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
			return err
		} else {
			...
			if err := desired.Apply(dbs.ApplyAppPrometheus(app, promData), app, r.Client, r.Scheme, log); err != nil {
				r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
				return err
			}
		}
	...
	if app.Spec.Dbs.Elastalert.Enabled {
		...
		if err := desired.Apply(dbs.ElastCreds(data), app, r.Client, r.Scheme, log); err != nil {
			r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
			return err
		}

		if err := desired.Apply(dbs.ElastAlert(), app, r.Client, r.Scheme, log); err != nil {
			r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
			return err
		}
	}

	if err := desired.Apply(dbs.AppDbsState(app), app, r.Client, r.Scheme, log); err != nil {
		r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
		return err
	}
	return nil
...
func (r *CnvrgAppReconciler) createPromDBCreds(app *mlopsv1.CnvrgApp) error {
	if app.Spec.Dbs.Prom.Enabled {
		...
		if err := desired.Apply(dbs.PromDBCreds(promSecretData), app, r.Client, r.Scheme, log); err != nil {
			r.updateStatusMessage(mlopsv1.Status{Status: mlopsv1.StatusError, Message: err.Error(), Progress: -1}, app)
			return err
		}
	}
...
func generateElastalertCreds(app *mlopsv1.CnvrgApp) (*desired.TemplateData, error) {
	...
	if err != nil {
		log.Error(err, "error generating elastalert hash")
		return nil, err
	}
...
func (r *CnvrgAppReconciler) getKibanaConfigSecretData(app *mlopsv1.CnvrgApp) (*desired.TemplateData, error) {
	...
	if err != nil {
		log.Error(err, "can't fetch es creds")
		return nil, err
	}
